...
<html>
  <head>
	...
	$(function() {
		...
		{display : '存档文件名',name : 'name',width : 200}
		],
		url : 'data.do?m=query',
			delayLoad :true,			
			width : '99.8%',
	...
	function backData(){
		wait = $.ligerDialog.waitting("正在备份，生成备份可能需要一段时间，请稍后...");
				$.ajax({url:'data.do?m=backup',type:"post",success : function(result) {
	...
	function delData(){
		...
		var row = $_grid.getSelectedRows()[0];
			...
			var data = {file:row.name}
				$.ajax({url:'data.do?m=delete',type:"post",data:data,success : function(result) {
					if (result.resultCode == '0') {
	...
	function download(row){
				window.open('data.do?m=download&file='+row.name);
	}
  ...
  <body>
    ...
    <input type="button" name="download" id="download" value="下载" />
