	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$_grid = $("#maingrid").ligerGrid({
					...
					url : 'materialFirst.do?m=queryApplys',
					delayLoad : true,
					width : '99.8%',
					height : '99%',
					pageSize : 20,
		...
		function readApplyByRow(row) {
			readWin = $.ligerDialog.open({
				title : "教材征订-阅读",
				url : 'materialFirst.do?m=initReadApply&id=' + row.id,
				width : 720,
				height : 330,
				isResize : true,
				isHidden : false,
		...
		function addApply() {
			addWin = $.ligerDialog.open({
				title : "首次教材征订-申请",
				url : 'materialFirst.do?m=addApply',
				width : 640,
				height : 320,
				isResize : true,
				isHidden : false,
		...
		function updateApplyByRow(row) {
			readWin = $.ligerDialog.open({
				title : "教材征订-修改",
				url : 'materialFirst.do?m=initUpdateApply&id=' + row.id,
				width : 720,
				height : 320,
				isResize : true,
				isHidden : false,
		...
		function deleteApplyByRow(row) {
			...
			var para = {id: id};
			$.ajax({url:"materialFirst.do?m=deleteApply",type:"POST",data:para,dataType:"json",async:false,success:deleteCallback});
		}
