  ...
  <script type="text/javascript">
        ...
        $(function()
        {
			...
			onSuccess:function(data){
					stationSel.selectValue('${data.Teacher.stationid}');
				}
			});
			...
			professionalSel=$('#professional').ligerComboBox({url:'teacherentrance.do?m=getprofessional',
			valueField:'id',
			textField:'name',                 
            width: $('#professional').parent().width()-2,
			selectBoxWidth:$('#professional').parent().width()-2,
			onSuccess:function(data){
					professionalSel.selectValue('${data.Teacher.professional_id}');
				}
			});
			...
			onSelected: function (newvalue)
			{
				departmentSel  = $('#dp2').ligerComboBox({url:'studententrance.do?m=dpid&'+'dpid='+newvalue,
					...
					onSuccess :function(data){
						...
						departmentSel.selectValue('${data.Teacher.dept_id}');
					}
					
				});	
			},
			onSuccess:function(data){
					schoolSel.selectValue('${data.Teacher.school_id}');
				}
			});
			  });
        function close(){
        ...
        var createWined;
        function createed(){
            createWined=$.ligerDialog.open({ title:"教职工教育信息登记", url: "teacherentrance.do?m=insertInited",timeParmName:"r", width: 750,height:500, showMax: true, showToggle: false, isResize: true, isHidden: false,slide: false,allowClose: false});
        }
        ...
        var createWinfl;
        function createfl(){
            createWinfl=$.ligerDialog.open({ title:"教职工教育信息登记", url: "teacherentrance.do?m=insertInitfl",timeParmName:"r", width: 750,height:500, showMax: true, showToggle: false, isResize: true, isHidden: false,slide: false,allowClose: false});
        }
        function createflClose(flag){       	
         ...
         var createWinwk;
        function createwk(){
            createWinwk=$.ligerDialog.open({ title:"教职工工作经历信息登记", url: "teacherentrance.do?m=insertInitwk",timeParmName:"r", width: 750,height:500, showMax: true, showToggle: false, isResize: true, isHidden: false,slide: false,allowClose: false});
        }
        
        function createwkClose(flag){       	
        		createWinwk.hide();        	
        }
		...
		function showgbMemberGridwk(){
			$gbMemberGridwk = $("#gbMemberGridwk").ligerGrid({
				columns:[
					         ...
					         {display:"开始时间", width: 100, name: "START_DATE",editor:{type:"date"}},
					         {display:"结束时间",name:"END_DATE",width:100,editor:{ type: "date" }},
					         {display:"操作", isSort: false, width: 80, render:function (row){return "<a style='text-decoration:underline;' href='javascript:deletewk(\"" + row.WK_EXPERIENCE + "\")'>删除</a> ";}}
						],
				url: "",
				delayLoad:false,
				width: "100%",  
				...
				onError:function(){
					...
					toolbar: { 
					items: [
				                  { text: "增   加  记 录", click: createwk, icon: "add" }
		...
		//删除一行工作信息
		function deletewk(){
			$.ligerDialog.confirm("是否删除选择【"+$gbMemberGridwk.selected[0].EXPERIENCE+"】?",function(result){
					if(result){
	    ...
	    poststred1.ID='<c:out value='${data.Teacher.id}'/>';
		...
		poststred1.TEL=new Array();
			...
			var datawk = $gbMemberGridwk.getData();
			if(datawk.length<1){
				$.ligerDialog.error('至少需要添加一个工作信息');
				
				return false;
			}
				$.each(datawk,function(i){
				
			poststred1.WK_EXPERIENCE.push(datawk[i].EXPERIENCE);
			poststred1.START_DATE.push(datawk[i].START_DATE);
			poststred1.END_DATE.push(datawk[i].END_DATE);
			...
			$.ajax({
						url:"teacherentrance.do?m=update",
						type:"post",
						dataType:"json",
						data:poststred1,
						cache:false,
  ...
  <body style="padding:10px">
    <form name="form1" method="post"  id="form1">
    	...
    	<table align="center" class="editView" cellspacing="1"  border="1">
 			<tr>
			   <td colspan="6" ><div align="center" class="editView td_div">教职工入职信息表</div></td>
			</tr>
			 	<tr>
 				 ...
 				 <tr>
				     ...
				     <td align="center" ><select id="dp1" name="dp1" title="学院">
                		<option value="<c:out value='${data.Teacher.school_id}'/>"> <c:out value='${data.Teacher.school_name}'/> </option></select>
				    </td>
				    <td align="center" ><select id="dp2" name="dp2" title="系部">
                		<option value="<c:out value='${data.Teacher.dept_id}'/>">  <c:out value='${data.Teacher.dept_name}'/>  </option></select>
                	</td>
                	
                	<td align="center" ><select id="professional" name="professional" title="职称">
                		<option value="<c:out value='${data.Teacher.professional_id}'/>"> <c:out value='${data.Teacher.professional}'/> </option></select>
                	</td>
                   
					
				  <td  align="center" ><input   type="text" name="NAME" id="NAME" title="教职工姓名"  value="<c:out value='${data.Teacher.name}'/>"/></td>
				    <td  align="center" ><select id="SEX" title="教职工性别" name="SEX"   style="width:173px;"/>
				    <option value="男">男</option>
                    <option value="女">女</option>
				    </select>
				    </td>
				    <td  align="center" ><input  type="text" name="CREDENTIAL_NUMBER" id="CREDENTIAL_NUMBER" title="教职工身份证号码"  value="<c:out value='${data.Teacher.credential_number}'/>"/></td>
 				 ...
 				 <tr>
 				 <td  align="center" ><input  type="text" name="NATION" id="NATION" title="员工民族"  value="<c:out value='${data.Teacher.nation}'/>"/></td>
				    <td  align="center" ><input  type="text" name="BORN_DATE" id="BORN_DATE" title="教职工出生日期"  value="<c:out value='${data.Teacher.born_date}'/>"/></td>
				    <td  align="center" ><input  type="text" name="BIRTH_PLACE" id="BIRTH_PLACE" title="教职工出生地"  value="<c:out value='${data.Teacher.birth_place}'/>"/></td>
				    <td  align="center" ><input type="text" id="HOUSE_REGISTE" title="教职工户籍" name="HOUSE_REGISTE" value="<c:out value='${data.Teacher.house_registe}'/>" /></td>
				    <td  align="center" ><input type="text" name="JOIN_DATE" id="JOIN_DATE" title="入学时间"  value="<c:out value='${data.Teacher.join_date}'/>"/></td>
				    <td  align="center" ><input  type="text" name="EMERGENCY_CONTACT_PERSON" id="EMERGENCY_CONTACT_PERSON" title="紧急联系人姓名"  value="<c:out value='${data.Teacher.emergency_contact_person}'/>"/></td>
				</tr>				 
				<tr>
			    <td width="120" class="editView_td_style" align="center" >紧急联系人电话</td>
				   ...
				   <td width="120" class="editView_td_style" align="center" >岗位</td>
				    ...
				    <tr>	
				   <td  align="center" ><input  type="text" name="EMERGENCY_CONTACT_TEL" id="EMERGENCY_CONTACT_TEL" title="紧急联系人电话"  value="<c:out value='${data.Teacher.emergency_contact_tel}'/>"/></td>
				  <td  align="center" ><input  type="text" name="POLITICAL_APPEARANCE" id="POLITICAL_APPEARANCE" title="政治面貌 "  value="<c:out value='${data.Teacher.political_appearance}'/>"/></td>
                	
                	<td align="center" ><select id="station" name="station" title="岗位">
                		<option value="<c:out value='${data.Teacher.stationid}'/>"> <c:out value='${data.Teacher.station}'/> </option></select>
                	</td>
