...
<html>
	...
	function openRead(row) {
		readWin = $.ligerDialog.open( {
			title : "试卷",
			url :  'file .do?m=readPaper&id=' + row.id,
			width : 650,
			height : 450,
			isResize : true,
			isHidden : false,
	...
	function del() {
		...
		$.ligerDialog.confirm("确认删除该试题 ?", function(result) {
			if (!result) {
				return;
			...
			//确认删除
				...
				//统计删除结果
				if (succesCount > 0) {
					$.ligerDialog.success('成功删除了该试题');
					refresh();
				}
				if (succesCount != rows.length) {
					$.ligerDialog.error((rows.length - succesCount) + '该试题删除失败');
				}
	...
	<body>
		<div class="button_panel">
			...
			<input type="button" name="read" id="read" value="保存" />
