...
<head>
	...
	<script type="text/javascript">
			...
			$_grid = $("#maingrid").ligerGrid(
					{
						...
						url : 'teacherOfHistory.do?m=getEvaluations',
						delayLoad : true,
						width : '99.8%',
						height : '99%',
						pageSize : 20,
		...
		function initYears() {
			$.ajax({url:"teacherOfHistory.do?m=getYears",type:"POST",dataType:"json",async:false,success:loadYears});
		}
		...
		function readEvaluationsByRow(row) {
			if($("#categoryAction").val() == 1) {
				readWin = $.ligerDialog.open({
					...
					url : 'teacherOfHistory.do?m=readEvaluations&evaluationSettingId=' + row.evaluationId,
					width : 960,
					height : 560,
					isResize : true,
					isHidden : false,
				...
				readWin = $.ligerDialog.open({
					title : "课堂教学质量评估-" + row.course,
					url : 'teacherOfHistory.do?m=readEvaluation&evaluationSettingId=' + row.evaluationId,
					width : 960,
					height : 560,
					isResize : true,
					isHidden : false,
