 ...
 */
@Repository
public class MaterialDirectorFirstDao extends BaseDao
{
	/**
	 * 方法名： queryApplys
	 * 建立者： 孟兆祥
	...
	public PagingJson queryApplys(String year, String deptId, int pageIndex, int pageSize)
	{
		try
		{
			String countSql = "SELECT COUNT(F.ID) " +
  										...
  										"WHERE F.ID = O.MATERIAL_ID AND APPLY_YEAR = ? AND APPLY_RESULT != '1' AND USER_ID IN " +
  										"(SELECT USER_ID FROM T_USER WHERE DEPT_ID = ?)";
	...
	public boolean approval(String id, String approvalAction)
	{
		try
		{
			String sql = "UPDATE T_MATERIALS_FIRST SET DIRECTOR = ? WHERE ID = ?";
