 ...
 */
@Repository
public class MaterialPurchaseFirstDao extends BaseDao
{
	/**
	 * 方法名： queryApplys
	 * 建立者： 孟兆祥
	...
	public PagingJson queryApplys(String year, String deptId, int pageIndex, int pageSize)
	{
		try
		{
			String countSql = "SELECT COUNT(M.ID) " +
										"FROM T_MATERIALS_FIRST M, T_MATERIALS_FIRST_ORDER O " + 
										"WHERE APPLY_YEAR = ? AND M.ID = O.MATERIAL_ID AND DEAN = '1' AND APPLY_RESULT != '1' AND USER_ID IN " +
	...
	public MaterialFirstItemVo readApply(String id)
	{
		...
		{
			SysLogUtil.error("阅读首次教材申请Dao层出错！", e);
			return null;
		}
	}
	...
	public boolean purchase(String materialId)
	{
		try
		{
			String sql = "UPDATE T_MATERIALS_FIRST_ORDER SET APPLY_RESULT = '1' WHERE MATERIAL_ID = ?";
	...
	public List<?> queryApplys(String year, String deptId) {
		try
		{
			...
			return this.jdbcTemplate.query(sql, para, new MaterialFirstItemRowMapper());
		}
		catch(Exception e)
		{
