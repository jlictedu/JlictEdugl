 ...
 */
@Repository
public class MaterialDirectorSelfDao extends BaseDao
{
	/**
	 * 方法名： queryApplys
	 * 建立者： 孟兆祥
	...
	public PagingJson queryApplys(String year, String deptId, int pageIndex, int pageSize)
	{
		try
		{
			String countSql = "SELECT COUNT(S.ID) " +
  										...
  										"WHERE S.ID = O.MATERIAL_ID AND APPLY_YEAR = ? AND APPLY_RESULT != '1' AND USER_ID IN (SELECT USER_ID FROM T_USER WHERE DEPT_ID = ?)";
			String querySql = "SELECT S.ID, ISBN, MATERIALS_NAME, EDITOR, PRESS, PRICE, COUNT, USE_CLASS, USER_ID, DIRECTOR, DIRECTOR_IDEA, DEAN, DEAN_IDEA, APPLY_RESULT, APPLY_DATE " +
					  					"FROM T_MATERIALS_SELF S, T_MATERIALS_SELF_ORDER O " + 
					  					"WHERE S.ID = O.MATERIAL_ID AND APPLY_YEAR = ? AND APPLY_RESULT != '1' AND USER_ID IN (SELECT USER_ID FROM T_USER WHERE DEPT_ID = ?)";
	...
	public MaterialSelfVo readApproval(String id)
	{
		try
		{
			String sql = "SELECT S.ID, ISBN, MATERIALS_NAME, EDITOR, PRESS, PRICE, COUNT, USE_CLASS, USER_ID, DIRECTOR, DIRECTOR_IDEA, DEAN, DEAN_IDEA, APPLY_RESULT, APPLY_DATE " +
			  					"FROM T_MATERIALS_SELF S, T_MATERIALS_SELF_ORDER O " + 
	...
	public boolean approval(String id, String approvalAction)
	{
		try
		{
			String sql = "UPDATE T_MATERIALS_SELF SET DIRECTOR = ? WHERE ID = ?";
